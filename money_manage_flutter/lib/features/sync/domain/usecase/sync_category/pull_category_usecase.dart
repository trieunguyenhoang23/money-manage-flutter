...
@injectable
class PullCategoryUseCase {
  final CategorySyncRepository _categorySyncRepository;
  final SyncLocalStorage _syncLocalStorage;

  PullCategoryUseCase(this._categorySyncRepository, this._syncLocalStorage);

  int get _limit => SizeAppUtils().isTablet ? 20 : 10;

  Stream<SyncBatchProgress> execute() async* {
    final schema = SyncSchema.category;
    final String? lastSync = _syncLocalStorage.getLastSyncTime(schema);

    bool hasMore = true;
    int currentPage = 0;

    while (hasMore) {
      final result = await _categorySyncRepository.pullCategoryDelta(
